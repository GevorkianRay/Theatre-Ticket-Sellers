...
public class Seller implements Runnable {
	...
	public void run() {
		...
		boolean run = true;
		Customer hold = null;
		Customer hold2 = new Customer(999, "Temp");
		while (t.getElapsedTime() < 60) {
			...
			{	
					if(hold2.customerName.equals(hold.customerName))
					{
						continue;
					}
					else
					{
						System.out.println("00:" + t.getElapsedTime() +"      "  +name + " " +hold.customerName +" was turned away");
						hold.turnAway();
					}
			...
			for(int i = 0 ; i < listOfCustomers.size();i++) {
				if (!listOfCustomers.get(i).turnAway) {
					System.out.println("00:" + t.getElapsedTime() +"      "  +name + " " +listOfCustomers.get(i).customerName +" was turned away");
					listOfCustomers.get(i).turnAway();
				}
			}
